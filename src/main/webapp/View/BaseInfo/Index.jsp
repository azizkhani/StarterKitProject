...
<META http-equiv="Content-Type" content="text/html;charset=UTF-8">
	...
	<script language="javascript" type="text/javascript">
		...
		function fillTable() {
			...
			$.getJSON(restUrl + "/list/" + parentId + "/", JsonData, function(
					entities) {
				...
				$('#GridRowTemplate').tmpl(entitiesCache).appendTo(
						'#entityBody');
				if (resultNum == 0) {
					if (isSearchState)
				...
				$('table.grid tbody tr:not([th]):even').css('backgroundColor',
						'#DFEBF4');
				...
				if (confirm("آیا مطمئنید؟ شما قصد حذف اطلاعات زیر را دارید\n\r ")) {
					...
					$
							.ajax({
								...
								error : function(res) {
									showMessage('این مورد در جدول دیگری استفاده شده و امکان حذف وجود ندارد');
									Loader(false);
								}
							});
				}
		...
		function saveEntity() {
			...
			$.ajax({
				...
				error : function(res) {
					showMessage('کد یا شرح وارد شده تکراری است');
					Loader(false);
				}
			});
		}
